...
function sortedProductsDescending(product1, product2){
  return product2.rating - product1.rating;
}

app.get('/products/sort/popularity', (req, res) => {
  let productCopy  = products.slice() ;
  productCopy.sort(sortedProductsDescending);
  res.json({sortedProducts: productCopy});
});


function sortedProductsByPriceDesc(product1, product2){
  return product2.price - product1.price;
}

app.get('/products/sort/price-high-to-low', (req, res) => {
  let productCopy  = products.slice() ;
  productCopy.sort(sortedProductsByPriceDesc);
  res.json({sortedProducts: productCopy});
});


function sortedProductsByPriceAsc(product1, product2){
  return product1.price - product2.price;
}

app.get('/products/sort/price-low-to-high', (req, res) => {
  let productCopy  = products.slice() ;
  productCopy.sort(sortedProductsByPriceAsc);
  res.json({sortedProducts: productCopy});
});
...
app.get('/products/filter/ram1', (req, res) => {
  let ram = parseInt(req.query.ram);
  let result  =  filterByRam(products, ram) ;
  res.json({result});
});
